...
async function fetchDexPairsForMint(
  mint: string
): Promise<DexPair[]> {
  ...
  if (!res.ok) {
    throw new Error(
      `DexScreener error ${res.status} for ${mint}`
    );
  }
...
app.get("/api/wallet-info", async (req: Request, res: Response) => {
  ...
  if (!address) {
    return res
      .status(400)
      .json({ error: "Missing address query param" });
  }
  ...
  try {
    ...
    // 2) SPL tokens (alle balances) – uiAmount zelf uitrekenen
    ...
    const tokens = tokenAccounts.value
      .map((ta) => {
        ...
        // uiAmount zelf berekenen, NIET vertrouwen op tokenAmount.uiAmount
        let uiAmount = 0;
        try {
        ...
        // Alleen tokens tonen met > 0 balance
      ...
      .filter((t) => t !== null)
      // sorteer op grootste balance eerst
...
app.get("/api/token-info", async (req: Request, res: Response) => {
  ...
  if (!mint) {
    return res
      .status(400)
      .json({ error: "Missing mint query param" });
  }
  ...
  try {
    const parsed =
      await connection.getParsedAccountInfo(mintKey, "confirmed");
    if (!parsed.value) {
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const raydiumPairs = pairs.filter(
        (p) =>
          p.chainId === "solana" && p.dexId.toLowerCase() === "raydium"
      );
